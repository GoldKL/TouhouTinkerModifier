...
public class MasterSparkRenderer extends EntityRenderer<MasterSparkVisualEntity> {

    public static final ModelLayerLocation MODEL_LAYER_LOCATION = new ModelLayerLocation(TouhouTinkerModifier.getResource("master_spark_model"), "main");
    private static final ResourceLocation TEXTURE_CORE = TouhouTinkerModifier.getResource("textures/entity/master_spark/core.png");
    private static final ResourceLocation TEXTURE_OVERLAY = TouhouTinkerModifier.getResource("textures/entity/master_spark/overlay.png");

    private final ModelPart body;

    public MasterSparkRenderer(Context context) {
        super(context);
    ...
    @Override
    public boolean shouldRender(MasterSparkVisualEntity pLivingEntity, Frustum pCamera, double pCamX, double pCamY, double pCamZ) {
        return true;
    }

    @Override
    public void render(MasterSparkVisualEntity entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int light) {
        poseStack.pushPose();
        float scalar = .25f;
        ...
        for (float i = 0; i < entity.distance * 4; i += length) {
            ...
            {
                poseStack.pushPose();
                float expansion = Mth.clampedLerp(1.2f, 0, 0) * 6;
            ...
            {
                poseStack.pushPose();
                float expansion = Mth.clampedLerp(1, 0, 0) * 6;
                poseStack.scale(expansion, 1, expansion);
    ...
    @Override
    public ResourceLocation getTextureLocation(MasterSparkVisualEntity entity) {
        return TEXTURE_CORE;
    }
}